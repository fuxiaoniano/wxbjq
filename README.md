...
- 独立的视频批量下载页，支持标准抖音视频页、`v.douyin.com` 分享短链、包含短链的整段分享文案和 `adsmind.gdtimg.com` 直链；支持批量任务、文件夹选择与自定义重命名，抖音任务采用顺序解析、双路下载流水线，并在页脚显示低调的浏览器去重访问人数。
...
- `2.3.4` 增加编辑器访问人数统计，统计结果仅在受管理员权限保护的管理后台中显示；编辑器返回主站固定指向 `https://fuxiaonian.net/`。
- `2.3.3` 为视频下载页增加访问人数统计；同一浏览器只计一次，服务端仅保存累计数字，不记录 IP、账号或访问明细。
- `2.3.2` 支持直接粘贴包含 `v.douyin.com` 短链的抖音分享文案，受控展开官方跳转后继续解析无 App Logo 原视频。
- `2.3.1` 将抖音批量任务拆分为解析和下载两个阶段；上一条解析完成后立即解析下一条，已解析任务由两个下载 worker 并行传输。
