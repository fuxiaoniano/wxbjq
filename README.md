...
项目现已包含邮箱账号、SMTP 邮件验证、安全会话，可扩展的套餐、会员、Feature、Entitlement 和配额体系，以及微信公众号多账号绑定。当前仍使用本地 JSON 数据文件，不包含支付；保存到公众号草稿箱属于下一阶段能力。
...
现有草稿和模板接口为了兼容旧版，默认仍保持原有访问行为，并未自动变成账号私有数据。`public-stateless` 模式通过关闭服务器写入，避免公开访客写入共享草稿和模板。新增的会员和管理员接口已经执行独立身份与权限校验。
